...
export function Footer() {
  const { config: rifaConfig, loading } = useRifaConfigState();

  return (
    <footer className="border-t border-white/10 px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
        <div>
          {loading ? <Skeleton className="h-8 w-44" /> : <p className="font-heading text-2xl font-bold text-foreground">{rifaConfig.sellerName}</p>}
          <p className="mt-2 max-w-xl text-xs leading-5 text-white/45">
            Compra tus entradas digitales. Entrega inmediata.
          </p>
        </div>
        <div className="flex gap-3">
          <a href={loading ? "#" : rifaConfig.socialLinks.instagram} target="_blank" rel="noreferrer" className="grid size-11 place-items-center rounded-full border border-white/12 text-white/70 transition hover:border-lime-300 hover:text-lime-300">
          ...
          <a href={loading ? "#" : rifaConfig.socialLinks.whatsapp} target="_blank" rel="noreferrer" className="grid size-11 place-items-center rounded-full border border-white/12 text-white/70 transition hover:border-lime-300 hover:text-lime-300">
            <svg viewBox="0 0 24 24" aria-hidden="true" className="size-5 fill-current">
